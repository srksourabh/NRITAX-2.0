(function(){
const { Wordmark, Button, StatusPill } = window.NRITAX20DesignSystem_c86cd4;

function SiteNav({ page, go }) {
  const items = [['home', 'How it works'], ['pricing', 'Pricing'], ['guide', 'Guides']];
  return (
    <header style={{ background: 'var(--ink)', color: 'var(--surface)', height: 'var(--shell-header-height)', display: 'flex', alignItems: 'center', gap: '16px', padding: '0 var(--gutter-mobile)', overflow: 'hidden' }}>
      <button type="button" onClick={() => go('home')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', flex: '0 0 auto' }}><Wordmark /></button>
...
function SiteFooter() {
  ...
  return (
    <footer className="ntx-section" style={{ background: 'var(--primary-800)', color: 'rgba(252,253,252,0.82)' }}>
      <div style={{ maxWidth: 'var(--content-max-app)', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(160px, 100%), 1fr))', gap: '32px' }}>
          ...
          {[['Product', ['How it works', 'Pricing', 'Expert filing', 'Security']], ['Guides', ['Old vs new regime', 'Form 16 explained', '26AS and AIS', 'Belated returns']], ['Company', ['About', 'Contact', 'Terms', 'Privacy']]].map(([h, links]) => (
            <div key={h} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              {links.map((l) => <a key={l} href="#" style={{ fontSize: 'var(--body-sm)', color: 'rgba(252,253,252,0.82)', textDecoration: 'none' }}>{l}</a>)}
            </div>
          ))}
        </div>
        ...
        <p style={{ fontFamily: 'var(--font-figure)', fontSize: 'var(--statute)' }}>© 2026 NRITAX. Figures shown are illustrative. Registration numbers are placeholders pending real credentials.</p>
      </div>
    </footer>
  );
}
